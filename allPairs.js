...
exports.getTestcases = function (categories) {
  ...
  pairs.map(function (pair) {
    ...
    if (pair.getNbrInTestcase() === 0) {
      var testdata = createPairwiseTestcase(pair, keys, pairs);
  ...
  //  remove testcases without at least one 'in test case = 1' 
    ...
    pairsInTestcase.map(function (pairInTestcase) {
      pairs.map(function (pair) {
        if (pairInTestcase.key1() === pair.key1() &&
          pairInTestcase.value1() === pair.value1() &&
          pairInTestcase.key2() === pair.key2() &&
          pairInTestcase.value2() === pair.value2() &&
          pair.getNbrInTestcase() === 1) {
          keepTestcase = true;
        };
      });
    });
